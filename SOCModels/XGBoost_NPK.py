...
# ---------------------------------------------------------------------------
# Parameter grid — regularised for small datasets (~51 samples)
# ---------------------------------------------------------------------------
PARAM_GRID = {
    ...
    for root, _, files in os.walk(image_base_dir):
        for fname in files:
            if fname.lower().endswith((".jpg", ".jpeg", ".png", ".bmp")):
                key = os.path.splitext(fname.lower())[0]   # [0] — name only, not tuple
                if key in index:
...
def train_xgb_npk(
    ...
    for nutrient, y, model_path, num_class in nutrients:
        ...
        # Report on LOO predictions (honest — not training-set predictions)
